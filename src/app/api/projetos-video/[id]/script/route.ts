// app/api/projetos-video/[id]/script/route.ts
...
export async function PUT(request: NextRequest, { params }: Params) {
  try {
    ...
    const { conteudo, concluido } = await request.json();
    ...
    const projetoAtualizado = await db.projetoVideo.update({
      ...
      data: {
        etapaScript: JSON.stringify(etapaScript),
        statusGeral: calcularStatusGeral(projeto, { etapaScript }),
      },
    });
