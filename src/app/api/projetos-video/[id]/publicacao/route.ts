// app/api/projetos-video/[id]/publicacao/route.ts
...
export async function PUT(request: NextRequest, { params }: Params) {
  try {
    ...
    const { concluido, urlPublicada, dataPublicacao } = await request.json();
    ...
    // Atualizar etapa Publicação
    const etapaPublicacao = {
    ...
    const projetoAtualizado = await db.projetoVideo.update({
      ...
      data: {
        etapaPublicacao: JSON.stringify(etapaPublicacao),
        statusGeral: calcularStatusGeral(projeto, { etapaPublicacao }),
      },
    });
...
function calcularStatusGeral(projeto: any, etapasAtualizadas: any) {
  const etapas = {
    ...
    publicacao:
      etapasAtualizadas.etapaPublicacao ||
