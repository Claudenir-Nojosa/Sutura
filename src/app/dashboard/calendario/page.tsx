...
interface Atividade {
  ...
  data: Date;
  concluida: boolean;
  categoria: "script" | "thumbnail" | "edicao" | "descricao_seo" | "publicacao";
  ordem?: number;
...
const CATEGORIAS = {
  script: {
    ...
    corConcluida: "bg-red-100 text-red-800 border-red-200",
    corEscura: "bg-red-500",
  },
  thumbnail: {
    label: "Thumbnail",
...
export default function CalendarioPage() {
  ...
  const [novaAtividade, setNovaAtividade] = useState({
    ...
    categoria:
      "script" as "script" | "thumbnail" | "edicao" | "descricao_seo" | "publicacao",
  ...
  useEffect(() => {
    if (!isReordering) {
      document
        .querySelectorAll(
          ".opacity-50, .border-2, .border-red-400, .scale-105, .bg-red-100, .dark\\:bg-red-900\\/20"
        )
        .forEach((el) => {
          el.classList.remove(
            "opacity-50",
  ...
  const handleVerticalDragLeave = (e: React.DragEvent) => {
    setDragOverAtividade(null);
    e.currentTarget.classList.remove("border-2", "border-red-400");
  };
  ...
  const handleVerticalDrop = async (
    ...
    document
      .querySelectorAll(".opacity-50, .border-2, .border-red-400")
      .forEach((el) => {
        el.classList.remove("opacity-50", "border-2", "border-red-400");
      });
    ...
    return atividadesOrdenadas.map((atividade) => {
      ...
      return (
        <div
          ...
          ${dragOverAtividade === atividade.id ? "border-2 border-red-400 bg-red-50 dark:bg-red-900/20" : ""}
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < startingDayOfWeek; i++) {
      ...
      days.push(
        <div
          key={`empty-${i}`}
          className={`min-h-24 sm:min-h-28 md:min-h-36 lg:min-h-40 p-1 sm:p-1.5 md:p-2 border dark:border-red-900/40 ${
            isWeekend(dayIndex)
              ? "dark:bg-gray-900 bg-red-50/30"
              : "dark:bg-gray-950 bg-white"
          } text-gray-400 dark:text-red-500/70`}
        >
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        <div
          key={`day-${i}`}
          className={`day-cell min-h-24 sm:min-h-28 md:min-h-36 lg:min-h-40 p-1 sm:p-1.5 md:p-2 border dark:border-red-900/40 transition-all flex flex-col
          ${isToday ? "dark:bg-red-900/30 border-red-300 dark:border-red-600" : ""}
          ${isWeekend(dayIndex) ? "dark:bg-gray-900 bg-red-50/30" : "dark:bg-gray-950 bg-white"}
            ...
            <div className="flex items-center gap-1">
              <span
                className={`text-xs sm:text-sm font-medium ${
                  isToday
                    ? "bg-gradient-to-br from-red-500 via-red-600 to-red-500 text-white rounded-full w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center shadow-[0_0_6px_0_rgba(192,132,252,0.7)]"
                    : "dark:text-red-100 text-gray-800"
                }`}
              >
                {i}
              </span>
        ...
        >
          <SheetHeader className="mb-4 text-center px-10">
            <SheetTitle className="flex flex-col items-center">
              ...
              {isToday && (
                <span className="text-xs text-red-600 mt-1 bg-red-100 px-2 py-1 rounded-full">
                  Hoje
                </span>
              )}
            </SheetTitle>
  ...
  return (
    <div className="container mx-auto py-4 md:py-8 px-2 md:px-4 mt-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h1 className="text-2xl md:text-3xl font-bold dark:text-red-100 text-black">
          Calendário
        </h1>
        <div className="flex items-center space-x-2 md:space-x-4 dark:bg-red-950/20 bg-white rounded-lg shadow-sm p-2 border dark:border-red-900/30 border-gray-200 w-full md:w-auto justify-between">
            ...
            >
              <PopoverTrigger asChild>
                <Button
                  ...
                  className="dark:border-red-900/30 dark:hover:bg-red-900/20"
                >
                  <Filter className="h-3 w-3 md:h-4 md:w-4 mr-1 md:mr-2" />
                  <span className="hidden md:inline">Filtrar</span>
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-80 dark:bg-black dark:border-red-900/30">
                <div className="grid gap-4">
                  <div className="space-y-2">
                    <h4 className="font-medium dark:text-red-100">
                      Categorias
                    </h4>
                    <div className="grid gap-2">
                      {Object.entries(CATEGORIAS).map(([key, categoria]) => (
                        <div key={key} className="flex items-center space-x-2">
                          ...
                          <Label
                            htmlFor={`filter-${key}`}
                            className="text-sm font-normal dark:text-red-100"
                          >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Label
                        ...
                        className="text-sm font-normal dark:text-red-200"
                      >
                        Mostrar concluídas
                      </Label>
                    </div>
          ...
          <div className="flex items-center space-x-1 md:space-x-2">
            <Button
              ...
              size={isMobileView ? "sm" : "default"}
              onClick={prevMonth}
              className="dark:hover:bg-red-900/30 hover:bg-red-100 rounded-full p-1 md:p-2"
            >
              <ChevronLeft className="h-4 w-4 md:h-5 md:w-5 dark:text-red-300 text-red-600" />
            </Button>
            <h2 className="text-sm md:text-xl font-semibold dark:text-red-100 text-gray-700 min-w-[120px] md:min-w-[180px] text-center">
            ...
            <Button
              ...
              className="dark:hover:bg-red-900/30 hover:bg-red-100 rounded-full p-1 md:p-2"
            >
              <ChevronRight className="h-4 w-4 md:h-5 md:w-5 dark:text-red-300 text-red-600" />
            </Button>
          </div>
        </div>
      </div>
      ...
      {!isMobileView && (
        <div className="flex flex-wrap gap-4 mb-6 p-4 dark:bg-red-950/10 bg-white rounded-lg border dark:border-red-900/30 border-gray-200">
          {/* Categorias */}
          {Object.entries(CATEGORIAS).map(([key, categoria]) => (
            <div key={key} className="flex items-center gap-2">
              <div
                className={`w-3 h-3 rounded-full ${categoria.corEscura}`}
              ></div>
              <span className="text-sm dark:text-red-100">
                {categoria.label}
              </span>
            </div>
          ))}
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm dark:text-red-100">Concluído</span>
          </div>
        </div>
      )}

      {/* Grid do Calendário */}
      <div className="dark:bg-red-950/10 bg-white rounded-xl shadow-sm overflow-hidden border dark:border-red-900/30 border-gray-200">
        <div className="grid grid-cols-7 gap-px dark:bg-red-950/20 bg-gray-50 border-b dark:border-red-900/30 border-gray-200">
          {dayNames.map((day, index) => (
            <div
              ...
              className={`py-2 md:py-3 text-center font-medium text-xs md:text-sm 
                ${
                  isWeekend(index)
                    ? "dark:text-red-300 text-red-600 dark:bg-red-950/10 bg-red-50/30"
                    : "dark:text-red-100 text-gray-700"
                }`}
            >
      ...
      >
        <DialogContent className="sm:max-w-[425px] dark:bg-black dark:border-red-900/30 bg-white mx-2 md:mx-0">
          <DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <label
                htmlFor="nome"
                className="text-right dark:text-red-200 text-sm"
              >
                Nome:
              </label>
              <Input
                ...
                className="col-span-3 dark:bg-red-950/20 dark:border-red-900/30"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              >
                <SelectTrigger className="col-span-3 dark:bg-red-950/20 dark:border-red-900/30">
                  <SelectValue placeholder="Selecione a categoria" />
                </SelectTrigger>
                <SelectContent className="dark:bg-gray-950 dark:border-red-900/30">
                  <SelectItem
                    value="script"
                    className="dark:hover:bg-red-900/30 dark:focus:bg-red-900/30 cursor-pointer"
                  >
                    Script
                  </SelectItem>
                  <SelectItem
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                readOnly
                className="col-span-3 dark:bg-red-950/30 dark:border-red-900/30"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <label
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              className="bg-gradient-to-r from-red-600 to-red-600 hover:from-red-500 hover:to-red-500 text-white"
              onClick={
      ...
      <Dialog open={openTempoDialog} onOpenChange={setOpenTempoDialog}>
        <DialogContent className="sm:max-w-[425px] dark:bg-black dark:border-red-900/30 bg-white mx-2 md:mx-0">
          <DialogHeader>
            <DialogTitle className="dark:text-red-100 flex items-center gap-2">
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {atividadeParaTempo.historicoTempo.map(
                        (sessao: any, index: number) => (
                          <div
                            key={index}
                            className="flex justify-between items-center text-xs p-2 bg-gray-50 dark:bg-red-800 rounded"
                          >
                            <span>
